...
@section('title', 'Pusat Pengetahuan')

@section('content')

    @php
        $breadcrumbs = [['name' => 'Beranda', 'url' => route('home')], ['name' => 'Pusat Pengetahuan']];
    @endphp
    <x-hero title="Pusat Pengetahuan"
        subtitle="Jelajahi berbagai artikel, panduan, dan sumber daya untuk meningkatkan kesadaran dan pemahaman Anda."
        :breadcrumbs="$breadcrumbs" />

    <div class="educational-container mt-5">
            ...
            <div id="load-more-container" class="text-center mt-5">
                @if ($latestArticles->hasMorePages())
                    <button id="load-more-btn" class="btn btn-danger"
                        data-next-page="{{ $latestArticles->nextPageUrl() }}">
                        Muat Lebih Banyak
                    </button>
                @endif
            </div>
