...
<div class="container-fluid p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 fw-semibold">Kelola Staff</h1> {{-- GANTI JUDUL --}}
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="usersTable" class="table table-hover align-middle" style="width:100%"> {{-- GANTI ID TABEL --}}
                    ...
                    <tbody>
                        @foreach($users as $user) {{-- GANTI VARIABEL LOOP --}}
                        <tr>
                            <td>{{ $user->id }}</td>
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->username }}</td>
                            ...
                            <td>
                                <a href="{{ route('admin.users.edit', $user) }}" class="btn btn-sm btn-outline-primary" title="Edit"> {{-- GANTI ROUTE & VARIABEL --}}
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="d-inline delete-user-form"> {{-- GANTI ROUTE & CLASS & VARIABEL --}}
                                    @csrf
                                    @method('DELETE')
