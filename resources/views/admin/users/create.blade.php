...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-header d-flex justify-content-between align-items-center">
            <h1 class="h3 fw-semibold p-2 mb-0">Tambah Staff Baru</h1>
        </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.users.store') }}" method="POST"> {{-- GANTI ROUTE --}}
                ...
                <div class="mb-3">
                    <label for="role" class="form-label">Role (Izin Akses) <span class="text-danger">*</span></label>
                    {{-- GANTI NAME & ID --}}
                    <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                        {{-- GANTI NAME & ID --}}
                        <option value="" disabled {{ old('role') ? '' : 'selected' }}>Pilih Role</option>
                        <option value="global_admin" {{ old('role') == 'global_admin' ? 'selected' : '' }}>Global Admin
                        </option>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('staff_status') is-invalid @enderror" id="staff_status"
                        ...
                        <option value="" disabled {{ old('staff_status') ? '' : 'selected' }}>Pilih Status Staff
                        </option>
                        <option value="Dosen" {{ old('staff_status') == 'Dosen' ? 'selected' : '' }}>Dosen</option>
                        <option value="Mahasiswa" {{ old('staff_status') == 'Mahasiswa' ? 'selected' : '' }}>Mahasiswa
                        </option>
